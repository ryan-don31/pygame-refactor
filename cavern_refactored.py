...
class MenuScreen:
    ...
    def update(self, input_state):
        ...
        if input_state.fire_pressed:
            self.app.change_screen(State.PLAY)
...
class PlayScreen:
    ...
    def update(self, input_state):        
        ...
        if not self.paused:
            ...
            if self.game.player.lives < 0:
                self.app.change_screen(State.GAME_OVER)
    ...
    def draw(self):
        ...
        if self.paused:
            screen.draw.text("PAUSED", center=(400, 300), fontsize=80)
...
class GameOverScreen:
    ...
    def update(self):
        if keyboard.space:
            self.app.change_screen(State.MENU)

    def draw(self):
        screen.draw.text("GAME OVER", center=(WIDTH/2, 200), fontsize=60)
...
class InputManager:
    ...
    def build(self):
        ...
        if keyboard.space and not self.prev_space:
            state.fire_pressed = True

        if keyboard.up and not self.prev_up:
            state.jump_pressed = True

        if keyboard.p and not self.prev_p:
            state.pause_pressed = True
        self.prev_p = keyboard.p

        self.prev_space = keyboard.space
